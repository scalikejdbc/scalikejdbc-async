...
"""
  ...
  val generate: Seq[GeneratedFile] = {
    ...
    Seq(
      (2 to max).flatMap { n =>
        Seq(
          GeneratedFile(s"AsyncOneToManies${n}SQLToOption.scala", AsyncOneToManiesSQLToOption(n)),
          GeneratedFile(s"AsyncOneToManies${n}SQLToIterable.scala", AsyncOneToManiesSQLToIterable(n)),
          GeneratedFile(s"AsyncOneToManies${n}SQLToList.scala", AsyncOneToManiesSQLToList(n))
        )
      },
      {
        val methods = (2 to max).flatMap{ n =>
  ...
  def extractTo(n: Int) = (1 to n).map("underlying.extractTo" + _).mkString(", ")
...
class AsyncOneToManies${n}SQLToOption[A, ${B1_to_BN(n)}, Z](val underlying: OneToManies${n}SQLToOption[A, ${B1_to_BN(n)}, HasExtractor, Z])
  extends AnyVal
...
class AsyncOneToManies${n}SQLToIterable[A, ${B1_to_BN(n)}, Z](val underlying: OneToManies${n}SQLToIterable[A, ${B1_to_BN(n)}, HasExtractor, Z])
  extends AnyVal
...
class AsyncOneToManies${n}SQLToList[A, ${B1_to_BN(n)}, Z](val underlying: OneToManies${n}SQLToList[A, ${B1_to_BN(n)}, HasExtractor, Z])
  extends AnyVal
...
"""
  ...
  final def oneToManies${n}Traversable[A, ${B1_to_BN(n)}, Z](
    ...
    ${(1 to n).map(i => s"extractTo${i}: WrappedResultSet => Option[B${i}]").mkString(", ")}
  )(
  ...
  def oneToManies${n}Iterable[A, ${B1_to_BN(n)}, Z](
    ...
    withListeners(statement, _parameters) {
      ...
      def processResultSet(result: LinkedHashMap[A, (${seqB(n)})], rs: WrappedResultSet): LinkedHashMap[A, (${seqB(n)})] = {
        val o = extractOne(rs)
        ...
        result.keys.find(_ == o).map { _ =>
          (${(1 to n).map("to" + _).mkString(" orElse ")}).map { _ =>
            ...
            result += (o -> (
              ${(1 to n).map(i => s"to${i}.map(t => if (ts${i}.contains(t)) ts${i} else ts${i} :+ t).getOrElse(ts${i})").mkString(", ")}
            ))
          }.getOrElse(result)
        }.getOrElse {
          result += (
            o -> (
              ${(1 to n).map(i => s"to${i}.map(Vector(_)).getOrElse(Vector.empty)").mkString(", ")}
            )
          )
        }
      }
      ...
      connection.sendPreparedStatement(statement, _parameters: _*).map { result =>
        result.rows.map { ars =>
          new AsyncResultSetIterator(ars).foldLeft(
            ...
            case (one, (${(1 to n).map("t" + _).mkString(", ")})) => transform(one, ${(1 to n).map("t" + _).mkString(", ")})
          }
        }.getOrElse(Nil)
      }
    }
  ...
  def makeOneToManiesSQLToOptionAsync(n: Int): String = {
    s"""
  final implicit def makeOneToManies${n}SQLToOptionAsync[A, ${B1_to_BN(n)}, Z](sql: OneToManies${n}SQLToOption[A, ${B1_to_BN(n)}, HasExtractor, Z]): AsyncOneToManies${n}SQLToOption[A, ${B1_to_BN(n)}, Z] =
    new AsyncOneToManies${n}SQLToOption[A, ${B1_to_BN(n)}, Z](sql)"""
  }

  def makeOneToManiesSQLToIterableAsync(n: Int): String = {
    s"""
  final implicit def makeOneToManies${n}SQLToIterableAsync[A, ${B1_to_BN(n)}, Z](sql: OneToManies${n}SQLToIterable[A, ${B1_to_BN(n)}, HasExtractor, Z]): AsyncOneToManies${n}SQLToIterable[A, ${B1_to_BN(n)}, Z] =
    new AsyncOneToManies${n}SQLToIterable[A, ${B1_to_BN(n)}, Z](sql)"""
  }

  def makeOneToManiesSQLToListAsync(n: Int): String = {
    s"""
  final implicit def makeOneToManies${n}SQLToListAsync[A, ${B1_to_BN(n)}, Z](sql: OneToManies${n}SQLToList[A, ${B1_to_BN(n)}, HasExtractor, Z]): AsyncOneToManies${n}SQLToList[A, ${B1_to_BN(n)}, Z] =
