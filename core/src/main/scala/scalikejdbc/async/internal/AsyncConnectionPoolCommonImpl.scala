...
abstract class AsyncConnectionPoolCommonImpl[T <: ConcreteConnection](
...
) extends AsyncConnectionPool(settings)
  with JasyncConfiguration
  with LogSupport {

  private[this] val factory = factoryF(
    configuration(url, user, password, settings.connectionSettings)
  )
  ...
  override def giveBack(conn: NonSharedAsyncConnection): Unit = conn match {
    case conn: NonSharedAsyncConnectionImpl =>
      pool.giveBack(conn.underlying.asInstanceOf[T])
    case _ =>
      log.debug("You don't need to give back this connection to the pool.")
  }
}
