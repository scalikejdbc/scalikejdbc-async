...
import scalikejdbc.async.internal.DateConvert
...
private[scalikejdbc] class R2DBCRowResultSet(
  var currentRow: Option[XPTO],
  var other: Iterable[XPTO]
) extends ResultSet {
  var closed = false
    ...
    case s: String =>
      {
        try s.toInt != 0
          ...
          case e: NumberFormatException => !s.isEmpty
        }
