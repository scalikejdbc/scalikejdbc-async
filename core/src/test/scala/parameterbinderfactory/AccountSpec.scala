...
class AccountSpec
  ...
  case class PersonWithAccounts(
    person: Person,
    accounts: collection.Seq[Account]
  )
  ...
  val account2 =
    Account(accountId2, personId, accountDetails, Option(accountId))
  ...
  it should "select person joining account with custom binder sync" in {
    ...
    val result: Option[PersonWithAccounts] = NamedDB("mysql").readOnly {
      implicit s =>
        withSQL {
  ...
  it should "select person joining account with custom binder async" in {
    ...
    val resultFuture: Future[Option[PersonWithAccounts]] =
  ...
  it should "select account joining person with custom binder sync" in {
    val result: Option[AccountAndPerson] = NamedDB("mysql").readOnly {
      implicit s =>
        withSQL {
  ...
  it should "select account joining person with custom binder async" in {
    val resultsFuture: Future[Option[AccountAndPerson]] =
      NamedAsyncDB("mysql").withPool { implicit s =>
        withSQL {
          selectFrom(Account as acc)
            .join(Person as p)
