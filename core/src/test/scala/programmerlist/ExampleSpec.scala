...
class ExampleSpec
  ...
  it should "work with ConnectionPool" in {
    val findAllFuture: Future[List[Programmer]] = AsyncDB.withPool {
      implicit session =>
        Programmer.findAll()
    }
  ...
  it should "work within a transaction" in {
    ...
    created.foreach { newCompany =>
      ...
      val withinTx: Future[Unit] = AsyncDB.localTx { implicit tx =>
        for {
          programmers <- Programmer.findAllBy(
            sqls.eq(p.companyId, newCompany.id)
          )
  ...
  it should "work for more than pool size items" in {
    ...
    val futureProgrammers = Future.sequence((1 to MaxPoolSize * 2) map { i =>
      Programmer.create(s"p$i", Some(1))
    })
