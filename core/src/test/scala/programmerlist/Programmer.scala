...
case class Programmer(
  ...
  def addSkill(skill: Skill)(implicit
    session: AsyncDBSession = AsyncDB.sharedSession,
    cxt: EC = ECGlobal
  ): Future[Int] = {
    insert
      .into(ProgrammerSkill)
      .namedValues(column.programmerId -> id, column.skillId -> skill.id)
  }

  def deleteSkill(skill: Skill)(implicit
    session: AsyncDBSession = AsyncDB.sharedSession,
    cxt: EC = ECGlobal
  ): Future[Int] = {
    delete
      .from(ProgrammerSkill)
      .where
      .eq(column.programmerId, id)
      .and
      .eq(column.skillId, skill.id)
  }
...
object Programmer extends SQLSyntaxSupport[Programmer] with ShortenedNames {
  ...
  def apply(p: ResultName[Programmer])(rs: WrappedResultSet): Programmer =
    new Programmer(
      id = rs.get[Long](p.id),
  ...
  def apply(p: SyntaxProvider[Programmer], c: SyntaxProvider[Company])(
    rs: WrappedResultSet
  ): Programmer = {
  ...
  def find(id: Long)(implicit
    ...
    withSQL {
      select
        ...
        .leftJoin(Skill as s)
        .on(sqls.eq(ps.skillId, s.id).and.isNull(s.deletedAt))
        .where
        .eq(p.id, id)
        .and
        .append(isNotDeleted)
  ...
  def findNoSkillProgrammers()(implicit
    session: AsyncDBSession = AsyncDB.sharedSession,
    cxt: EC = ECGlobal
  ): Future[List[Programmer]] = {
    withSQL {
  ...
  def findAllBy(where: SQLSyntax, withCompany: Boolean = true)(implicit
    session: AsyncDBSession = AsyncDB.sharedSession,
    cxt: EC = ECGlobal
  ): Future[List[Programmer]] = {
    withSQL {
  ...
  def save(m: Programmer)(implicit
    session: AsyncDBSession = AsyncDB.sharedSession,
    cxt: EC = ECGlobal
  ): Future[Programmer] = {
    withSQL {
